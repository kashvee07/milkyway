import { milkyway_backend } from "../../declarations1/milkyway_backend";
...
appointForm.addEventListener("submit", async (e) => {
  ...
  try {
    const getId = await milkyway_backend.addTodo(obj.textarea);
...
window.addEventListener("DOMContentLoaded", async () => {
  try {
    const getAll = await milkyway_backend.getAllTodo();
...
async function deleteTodo(userId) {
  try {
    ...
    await milkyway_backend.deleteTodo(userId);
